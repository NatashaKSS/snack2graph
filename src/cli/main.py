...
def generate_knowledge_graph(
    ...
    # 6. Analyse results to improve the process
    # References:
    # * LangChain describes its process here: https://python.langchain.com/docs/how_to/structured_output
    graph = extract_knowledge_graph_from_text(document, ontology)

    serialized_graph = graph.model_dump()
    path_to_results_file = "data/results/output.txt"
    with open(path_to_results_file, "w", encoding="utf-8") as f:
        f.write(json.dumps(serialized_graph, indent=2, ensure_ascii=False))

    typer.echo(f"Knowledge Graph extracted to '{path_to_results_file}'")
